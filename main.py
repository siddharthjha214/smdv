...
def check_pot_plugin_available():
    """Check if the yt-dlp PO Token plugin is installed and available."""
    try:
        import importlib
        importlib.import_module("yt_dlp_plugins")
        return True
...
def download_and_backup(video_id, url, title):
    ...
    # Multi-tier download strategy chain (PO-token-aware):
    #
    # The yt-dlp-get-pot plugin (when installed) automatically generates Proof-of-Origin tokens
    # for the 'web' client, which is the most reliable way to bypass YouTube's bot detection
    # on datacenter IPs (GitHub Actions, Azure, AWS, etc.).
    #
    # Strategy order:
    # 1. web client (PO token auto-generated by plugin) — primary, most reliable
    # 2. web + mweb fallback — try both web clients with PO tokens
    # 3. tv_downgraded + tv — YouTube TV API (no PO token needed, but may get bot-checked)
    # 4. Guest session (web without cookies) — bypasses flagged auth sessions
    # 5. visionos + android_vr — Apple/Meta device clients (bot bypass without PO token)
    # 6. mweb direct player — last resort, skips webpage to avoid 429 rate limits
    strategies = [
        {
            "name": "web client with PO token (primary — datacenter bot bypass)",
            "overrides": {"extractor_args": {"youtube": {"player_client": ["web"]}}},
        },
        {
            "name": "web + mweb client (PO token fallback)",
            "overrides": {"extractor_args": {"youtube": {"player_client": ["web", "mweb"]}}},
        },
        {
            "name": "tv_downgraded + tv client (TV API — no PO token)",
            "overrides": {"extractor_args": {"youtube": {"player_client": ["tv_downgraded", "tv"]}}},
        },
        {
            "name": "guest session (web without cookies — bypasses flagged auth)",
            "drop_cookiefile": True,
            "overrides": {"extractor_args": {"youtube": {"player_client": ["web"]}}},
        },
        {
            "name": "visionos + android_vr (device client bot bypass)",
            "drop_cookiefile": True,
    ...
    for attempt, strategy in enumerate(strategies):
        ...
        # Progressive backoff: wait longer between later attempts to avoid rate-limiting
        if attempt > 0:
            backoff = min(5 + (attempt * 3), 20)  # 5s, 8s, 11s, 14s, 17s
            print(f"Waiting {backoff}s before next attempt (anti-abuse backoff)...")
            time.sleep(backoff)
